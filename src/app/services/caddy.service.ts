...
export class CaddyService {
  ...
  public listCaddies: Array<{ num: number; name: string }> = [
    { num: 1, name: 'Caddy1' },
  ];
  ...
  public loadCaddyFromLocalStorage() {
    let myCaddiesList = localStorage.getItem(
      'ListCaddies_' + this.authService.authenticatedUser.username
    );
    this.listCaddies =
      myCaddiesList == undefined
        ? [{ num: 1, name: 'Caddy1' }]
        : JSON.parse(myCaddiesList);
    this.listCaddies.forEach((c) => {
      let cad = localStorage.getItem(
        'myCaddy_' + this.authService.authenticatedUser.username + '_' + c.name
      );
      this.caddies[c.name] =
        cad == undefined ? new Caddy(c.name) : JSON.parse(cad);
    });
  }
  ...
  saveCaddy() {
    ...
    localStorage.setItem(
      'myCaddy_' +
        this.authService.authenticatedUser.username +
        '_' +
        this.currentCaddyName,
      JSON.stringify(caddy)
    );
  }
  ...
  addNewCaddy(c: { num: number; name: string }) {
    ...
    localStorage.setItem(
      'ListCaddies_' + this.authService.authenticatedUser.username,
      JSON.stringify(this.listCaddies)
    );
  }
